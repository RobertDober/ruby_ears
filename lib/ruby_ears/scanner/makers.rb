module RubyEars
  module Scanner extend self
    ...
    def _make_block_quote(match, lnb)
      BlockQuote.new(
        content: match[2],
        indent: match[1].size,
        line: match.string,
    ...
    def _make_ol_list_item(match, lnb)
    ...
    def _make_table_line match, lnb
      leading = match[1]
      body = match
        .string
        .strip
    ...
    def _make_table_gfm_line(match, lnb)
      leading = match[1]
    ...
    def _make_ul_list_item(match, lnb)
    ...
    def _determine_if_header(columns)
      columns
        .all?{ |col| TableColumnRgx.match?(col) }
    end


    def _split_table_columns(line)
      line
        .split(%r{(?<!\\)\|})
        .map(&:strip)
        .map{ |col| col.gsub(%r{\\\|}, "|") }
